...
impl Conversation {
    ...
    pub fn send_message_stream(&mut self, prompt: &str, on_token: impl FnMut(&str)) -> Result<()> {
        self.send_message_stream_with_context(prompt, None, on_token)
    }

    /// Like [`send_message_stream`] but injects `extra_context` (e.g. RAG documents)
    /// between the system turn and the user message. The context is sent per-request
    /// and is not stored in conversation history.
    pub fn send_message_stream_with_context(
        &mut self,
        prompt: &str,
        ...
        self.send_raw_stream_with_context(&message_json, extra_context, on_token)
    }
    ...
    pub fn send_raw_stream(
        ...
        self.send_raw_stream_with_context(message_json, None, on_token)
    }

    pub fn send_raw_stream_with_context(
        &mut self,
        message_json: &str,
        extra_context: Option<&str>,
        ...
        let ctx_cstring = extra_context
            .map(|s| CString::new(s).map_err(|_| Error::NullPointer))
            .transpose()?;
        let ctx_ptr = ctx_cstring.as_ref().map_or(std::ptr::null(), |c| c.as_ptr());
        ...
        let _guard = cond.wait_while(guard, |d| !*d).unwrap_or_else(|e| e.into_inner());
